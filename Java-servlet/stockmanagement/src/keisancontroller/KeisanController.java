...
public class KeisanController extends HttpServlet{
	public void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException , IOException{
		...
		//Tableインスタンスを生成⇒year, month , goodsList occupantListをsetterに入れる
		Table table = new Table();
		table.setYear(year);
		table.setMonth(month);
		table.setGoodsList(goodsList);
		table.setOccupantList(occupantList);

		/*tableを引数に、1カ月の備品の使用個数テーブルを作成create table*/
		Keisan keisan = new Keisan();
		keisan.making(table);

		/*tableを引数に、1カ月の備品使用個数テーブルに入居者ID・入居者名を挿入*/
		keisan.adding(table);
