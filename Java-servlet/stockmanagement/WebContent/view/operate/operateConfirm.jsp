...
<% if(operation.getCountOut() == 0) {  %>
<p>入庫数：<%= operation.getCountIn() %></p>
<% }  %>
<p>出庫数：<%= operation.getCountOut() %></p>
<% if(operation.getStrSheds().equals("出庫") && reOccupant.getRoomNumber() == 0 && reOccupant.getOccupantName().equals("shareSpace")){ %>
<p>使用場所：共用部</p>
<% } else if (operation.getStrSheds().equals("出庫") && reOccupant.getRoomNumber() != 0 && !(reOccupant.getOccupantName().equals("shareSpace")) ){ %>
