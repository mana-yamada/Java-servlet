	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		if(parameter.equals("changeConfirm")) {
			try {
				...
				String floorChange = request.getParameter("occupantFloorChange");
				String strOccupantFloor = request.getParameter("occupantFloor");
				int occupantFloor = Integer.parseInt(strOccupantFloor);

				String roomNumberChange =  request.getParameter("roomNumberChange");
			...
			/*単価の入力欄で0~9以外の文字列が入っていた場合*/
				...
				/* 変更画面から変更確認画面(エラー表示)*/
				String errorMsg = "変更後の入居フロアか居室番号が正しく入力されていません。恐れ入りますがもう一度最初から編集画面に戻って操作をやり直してください。";
				HttpSession session = request.getSession();
				session.setAttribute("errorMsg", errorMsg);
				//forward
				String path = "/view/occupant/edit/changeConfirm.jsp";
